...
def get_structured_representation(predictions, annotations,
    ...
    structured_representation_cols = [
        predictions.columns[0], predictions.columns[1], predictions.columns[2]
    ]
    ...
    for i, image in enumerate(annotations):
        image_predicted_label = predictions.loc[predictions[
            predictions.columns[0]] == image].iloc[0][predictions.columns[2]]
        representation_df.loc[i, predictions.columns[0]] = image
        representation_df.loc[i, predictions.columns[1]] = predictions.loc[
            predictions[predictions.columns[0]] == image].iloc[0][
                predictions.columns[1]]
        representation_df.loc[i,
                              predictions.columns[2]] = image_predicted_label
        if representation_values == 'numeric':
            for semantic_feature in annotations[image]:
                representation_df.loc[
                    i, semantic_feature] = annotations[image][semantic_feature]
        else:
            for semantic_feature in annotations[image]:
    ...
    representation_df['classification_check'] = (representation_df[
        predictions.columns[1]] == representation_df[predictions.columns[2]])
    representation_df['classification_check'] = representation_df[
